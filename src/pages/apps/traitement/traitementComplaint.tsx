...
export default function TreatmentComplaint() {
  ...
  React.useEffect(() => {
    if (oneComplaint?.currentStep?.position) {
      const lastStep = oneComplaint.currentStep.position;

      // Adjust for zero-based index
      const adjustedStep = Math.max(0, lastStep - 1);

      // Update active step
      setActiveStep(adjustedStep);

      // Save to localStorage (optional)
      localStorage.setItem('activeStep', adjustedStep.toString());

      // Optional: Set proposed solution for step 4
      ...
      // Simulate loading
      setTimeout(() => setLoadingProvoked(false), 2000);
    }
  }, [oneComplaint]);
  ...
  const renderFormContent = (step: any, index: number) => {
    switch (index) {
      ...
      default:
        return <Typography>Vous êtes en attente. Merci de patienter jusqu'à ce que cette étape soit complétée.</Typography>;
    }
  };
                ...
                <Step key={step.position}>
                  ...
                  <StepContent>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      {activeStep > 0 && (
                        <Button onClick={handleBack} sx={{ mt: 1, mr: 1 }} disabled>
                          Retour
                        </Button>
                      )}
                    </Box>
