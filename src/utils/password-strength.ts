...
export const strengthColor: NumbColorFunc = (count) => {
  if (count < 2) return { label: 'Faible', color: 'error.main' };
  if (count < 3) return { label: 'Moyen', color: 'warning.main' };
  if (count < 4) return { label: 'Normal', color: 'warning.dark' };
  if (count < 5) return { label: 'Bon', color: 'success.main' };
  if (count < 6) return { label: 'Fort', color: 'success.dark' };
  return { label: 'Faible', color: 'error.main' };
};
